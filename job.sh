...
#SBATCH --job-name=qmix    # Nom du job
#SBATCH --output=qmix_%j.log   # Standard output et error log

#SBATCH --gres=gpu:1              # Number of GPUs
#SBATCH --cpus-per-task=1             # Utiliser un seul CPU pour cette tâche (job)
#SBATCH --mem=32G                  # Size of cpu memory
#SBATCH --time=0-12:00:00         # Max duration days-hours:minutes:seconds
...
cd ~/epymarl/
# run Python script
python src/main.py --hp_search=25 --seed=0 --config=qmix --env-config=gymma with env_args.key="mpe2-simple-tag-v3" env_args.pretrained_wrapper="RandomTag"
