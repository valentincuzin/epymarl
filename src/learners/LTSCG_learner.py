...
class LTSCGLearner(QLearner):
    ...
    def __init__(self, mac, scheme, logger, args):
        super(LTSCGLearner, self).__init__(mac, scheme, logger, args)
        ...
        self._model_kwargs['channels'] = self.args.batch_size
    ...
    def train(self, episode_sample: EpisodeBatch, t_env: int, episode_num: int):
        ...
        graph = graph.repeat(batch.batch_size,1,1)
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            self.logger.log_stat('State Graph loss', gstate_loss.item(), t_env)
            self.logger.log_stat('Obs Graph loss', gobs_loss.item(), t_env)
