...
class EGCNAgent(nn.Module):
    def __init__(self, input_shape, args):
        ...
        self.e = nn.Sequential(
            ...
            nn.ReLU())
        self.gnns: MessagePassing  = GCNConv(input_shape, args.hidden_dim)
        print("GNN SHAPE weight ###################\n",self.gnns.lin.weight.shape)
        self.hyper_rnn = nn.GRUCell(args.hidden_dim, input_shape*args.hidden_dim)
        self.fc2 = nn.Linear(args.hidden_dim, args.n_actions)
        print(f"\n\nDEBUG: total number of PARAMETERS for GNNAgent: {sum(p.numel() for p in self.parameters())} #####\n\n")

    def init_hidden(self):
        # make hidden states on same device as model
        return self.gnns.lin.weight.flatten().clone()

    def forward(self, inputs, hidden_states):
        graph_emb, weight_gcn = self._communication_process(inputs, hidden_states)
        with th.no_grad():
            self.gnns.lin.weight.copy_(weight_gcn[0].view(self.args.hidden_dim, self.input_shape))
        q = self.fc2(graph_emb)
        return q, weight_gcn
    ...
    def _communication_process(self, inputs, hidden_states):
        ...
        with th.no_grad():
            graph_emb = self.gnns(graphs.x, graphs.edge_index)
        print("hidden_states shape: ", hidden_states.shape)
        h_in = hidden_states.reshape(-1, self.input_shape*self.args.hidden_dim)
        print("h_in shape: ", h_in.shape)
        weight_gcn = self.hyper_rnn(graph_emb, h_in)
        print("weight_gcn shape: ", weight_gcn.shape)
        return graph_emb, weight_gcn
...
#         ).view((self.args.batch_size*self.args.n_agents, self.args.batch_size*self.args.n_agents))
