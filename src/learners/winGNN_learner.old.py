...
class winGNNLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        last_win = first_win = 0  # Last window idx and current windows index
        in_win = False
        for t in range(batch.max_seq_length - 1):
            if t != 0 and not in_win:  # from 1 and not already in a windows
                rand_win_size = random.randint(self.args.window_num -2, self.args.window_num + 2)  # Random window size
                first_win = random.randint(t, t + rand_win_size)  # Random select between i and i+window_size
                last_win = first_win + rand_win_size  # Last window idx
            if t==first_win:
                in_win = True
            if in_win and t != 0 and t < (batch.max_seq_length - rand_win_size):
                fast_weights = list(map(lambda p: p[0], zip(self.old_mac.partial_agent.parameters())))
                S_dw = [0] * len(fast_weights)
                losses = th.tensor(0.0).to(self.args.device)
                agent_outs = self.old_mac.forward(batch, t=t, test_mode=True, fast_weights=fast_weights)
                if t == last_win:
                    in_win = False  # we stop win
            else:
