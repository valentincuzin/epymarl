...
def _objective(trial, args_dict, _log):
    ...
    elif "meta" in param["name"]:
        param = hp.hp_wingnn_settings(trial, param)
    else:
...
def _run_optim(args_dict, _log):
    ...
    pruner = optuna.pruners.PatientPruner(optuna.pruners.PercentilePruner(25.0), patience=5)
...
def run_sequential(args, logger):
    ...
    runner.close_env()
    logger.log_max_return(args.name, args.env_args["key"], args.env_args, args.seed)
