...
#SBATCH --job-name=ltscg-hp    # Nom du job
#SBATCH --output=ltscg-hp_%j.log   # Standard output et error log
...
cd ~/epymarl/
# run Python script
python src/main.py --hp_search=25 --seed=0 --config=ltscg --env-config=gymma with env_args.key="mpe2-simple-tag-v3" env_args.pretrained_wrapper="RandomTag"